...
def get_memory(user_id, key):
    ...
    return user_memory[user_id].get(key)
...
async def handle_message(update: Update, context:ContextTypes.DEFAULT_TYPE):
    ...
    interest = get_memory(user_id,"interest")
    name = get_memory(user_id, "name")
    
    messages = get_user_messages(user_id)
    
    use_rag = is_raq_query(user_input)
    if "oil" in user_input.lower():
        save_memory(user_id, "interest", "oil change")
...
"""
    ...
    if interest:
        prompt += f"\nUser was previously interested in: {interest}"
